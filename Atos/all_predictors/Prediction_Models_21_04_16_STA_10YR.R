...
rf_mod_pred%>% accuracy(truth = STA_10YR, .pred_class)%>%bind_rows(rf_mod_pred%>% sens(truth = STA_10YR, .pred_class))%>%
  bind_rows(rf_mod_pred%>% spec(truth = STA_10YR, .pred_class))%>%bind_rows(rf_mod_pred%>% f_meas(truth = STA_10YR, .pred_class))
...
lr_mod_pred%>% accuracy(truth = STA_10YR, .pred_class)%>%bind_rows(lr_mod_pred%>% sens(truth = STA_10YR, .pred_class))%>%
  bind_rows(lr_mod_pred%>% spec(truth = STA_10YR, .pred_class))%>%bind_rows(lr_mod_pred%>% f_meas(truth = STA_10YR, .pred_class))
#Stack package
model_ensemble <- 
  stacks() %>%
...
ens_mod_pred <-svm_mod_pred%>%bind_rows(svm_mod_pred)%>%bind_rows(svm_mod_pred)
ens_mod_pred%>% accuracy(truth = STA_10YR, .pred_class)%>%bind_rows(ens_mod_pred%>% sens(truth = STA_10YR, .pred_class))%>%
  bind_rows(ens_mod_pred%>% spec(truth = STA_10YR, .pred_class))%>%bind_rows(ens_mod_pred%>% f_meas(truth = STA_10YR, .pred_class))
...
setwd("~/Usydney/Atos")
save(vips,ens_mod_pred, file = "STA_10YR_vips.RData")