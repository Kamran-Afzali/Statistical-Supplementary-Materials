...
rec_obj <- recipe(OD_upto5YR ~ ., data = train_data)
...
final_svm %>%
  ...
  vi( method = "permute", nsim = 10, target = "OD_upto5YR",
...
svm_mod_pred = final_svm %>%
  ...
  bind_cols(test_preped %>% select(OD_upto5YR))

svm_mod_pred%>% accuracy(truth = OD_upto5YR, .pred_class)%>%bind_rows(svm_mod_pred%>% sens(truth = OD_upto5YR, .pred_class))%>%
  bind_rows(svm_mod_pred%>% spec(truth = OD_upto5YR, .pred_class))%>%bind_rows(svm_mod_pred%>% f_meas(truth = OD_upto5YR, .pred_class))
#RF
rf_mod <- 
...
rf_mod_pred%>% accuracy(truth = OD_upto5YR, .pred_class)%>%bind_rows(rf_mod_pred%>% sens(truth = OD_upto5YR, .pred_class))%>%
  bind_rows(rf_mod_pred%>% spec(truth = OD_upto5YR, .pred_class))%>%bind_rows(rf_mod_pred%>% f_meas(truth = OD_upto5YR, .pred_class))
...
lr_mod_pred%>% accuracy(truth = OD_upto5YR, .pred_class)%>%bind_rows(lr_mod_pred%>% sens(truth = OD_upto5YR, .pred_class))%>%
  bind_rows(lr_mod_pred%>% spec(truth = OD_upto5YR, .pred_class))%>%bind_rows(lr_mod_pred%>% f_meas(truth = OD_upto5YR, .pred_class))
#Stack package
model_ensemble <- 
  stacks() %>%
...
ens_mod_pred%>% accuracy(truth = OD_upto5YR, .pred_class)%>%bind_rows(ens_mod_pred%>% sens(truth = OD_upto5YR, .pred_class))%>%
  bind_rows(ens_mod_pred%>% spec(truth = OD_upto5YR, .pred_class))%>%bind_rows(ens_mod_pred%>% f_meas(truth = OD_upto5YR, .pred_class))
...
setwd("~/Usydney/Atos")
save(vips,ens_mod_pred, file = "OD_upto5YR_vips.RData")
