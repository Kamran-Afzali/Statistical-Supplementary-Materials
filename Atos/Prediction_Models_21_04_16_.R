...
rec_obj <- recipe(STA_1YR ~ ., data = train_data)
...
final_svm %>%
  ...
  vi( method = "permute", nsim = 10, target = "STA_1YR",
...
svm_mod_pred = final_svm %>%
  ...
  bind_cols(test_preped %>% select(STA_1YR))

svm_mod_pred%>% accuracy(truth = STA_1YR, .pred_class)%>%bind_rows(svm_mod_pred%>% sens(truth = STA_1YR, .pred_class))%>%
  bind_rows(svm_mod_pred%>% spec(truth = STA_1YR, .pred_class))%>%bind_rows(svm_mod_pred%>% f_meas(truth = STA_1YR, .pred_class))
#RF
rf_mod <- 
...
rf_mod_pred%>% accuracy(truth = STA_1YR, .pred_class)%>%bind_rows(rf_mod_pred%>% sens(truth = STA_1YR, .pred_class))%>%
  bind_rows(rf_mod_pred%>% spec(truth = STA_1YR, .pred_class))%>%bind_rows(rf_mod_pred%>% f_meas(truth = STA_1YR, .pred_class))
...
lr_mod_pred%>% accuracy(truth = STA_1YR, .pred_class)%>%bind_rows(lr_mod_pred%>% sens(truth = STA_1YR, .pred_class))%>%
  bind_rows(lr_mod_pred%>% spec(truth = STA_1YR, .pred_class))%>%bind_rows(lr_mod_pred%>% f_meas(truth = STA_1YR, .pred_class))
#Stack package
model_ensemble <- 
  stacks() %>%
...
ens_mod_pred%>% accuracy(truth = STA_1YR, .pred_class)%>%bind_rows(ens_mod_pred%>% sens(truth = STA_1YR, .pred_class))%>%
  bind_rows(ens_mod_pred%>% spec(truth = STA_1YR, .pred_class))%>%bind_rows(ens_mod_pred%>% f_meas(truth = STA_1YR, .pred_class))
...
setwd("~/Usydney/Atos")
save(vips,ens_mod_pred, file = "STA_1YR_vips.RData")
